...
auto server_handler(vector<weathercast_t> &weather_data_ref)
{
    ...
    router->add_handler(
        ...
        [](const restinio::request_handle_t& req, rr::route_params_t ) {
            return req->create_response(restinio::status_ok())
                .append_header("Access-Control-Allow-Origin", "*")
                .append_header("Access-Control-Allow-Methods", "GET, POST, OPTIONS") // Methods supported on /weather
                .append_header("Access-Control-Allow-Headers", "Content-Type")
                .append_header("Access-Control-Max-Age", "86400") // Optional: caches preflight response
                .done();
        });
    ...
    router->add_handler(
        ...
        [](const restinio::request_handle_t& req, rr::route_params_t ) {
            return req->create_response(restinio::status_ok())
                .append_header("Access-Control-Allow-Origin", "*")
                .append_header("Access-Control-Allow-Methods", "GET, PUT, OPTIONS") // Methods supported on /weather/:id
                .append_header("Access-Control-Allow-Headers", "Content-Type")
                .append_header("Access-Control-Max-Age", "86400") // Optional: caches preflight response
                .done();
        });
